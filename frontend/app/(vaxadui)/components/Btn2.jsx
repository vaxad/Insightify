"use client"
export default function Btn1({ onClick, text, color, className, rounded }) {
  const dummyOnclik = () => {
  }
  const colorVariants = {
  ...
  return (
    <button className={` font-medium py-1 px-2 whitespace-nowrap text-lg ${roundedVariants[rounded?rounded:"full"]} border transition-all ${colorVariants[color?color:"blue"]} ${className}`} onClick={() => { onClick ? onClick() : dummyOnclik() }}>
